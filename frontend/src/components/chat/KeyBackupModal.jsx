...
const KeyBackupModal = ({ isOpen, onClose, mode = "backup", user, onRestoreSuccess }) => {
  ...
  const fingerprint = e2eeService.localCryptoKey?.fingerprint || "Not yet loaded";
  ...
  const handleGenerateFreshKey = async () => {
    if (!window.confirm("Warning: If you generate a new key pair, previously received encrypted messages that were encrypted for your old key will not be readable. Continue?")) {
      return;
    }

    setLoading(true);
    try {
      await e2eeService.generateAndRegisterNewKeys(user._id);
      toast.success("Generated and registered new encryption key pair");
      if (onRestoreSuccess) onRestoreSuccess();
      onClose();
    } catch (err) {
